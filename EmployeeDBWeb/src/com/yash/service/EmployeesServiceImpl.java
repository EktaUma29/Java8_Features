...
public class EmployeesServiceImpl implements EmployeesService {
	...
	public List<EmployeesModel> retrieveEmployees() {
		...
		try {
			...
			for (Employees employees : employeesList) {
				...
				employeesModel
						.setTotalSalary(employees.getSalary() + employees.getSalary() * employees.getCommissionPCT());
				employeesModel.setContactDetails(
						"Ph No:" + employees.getPhoneNumber() + "," + "Email:" + employees.getEmail());
	...
	public List<EmployeesModel> retrieveEmployees(int pageId, int total) {
		List<EmployeesModel> employeesModelList = new ArrayList<>();
		try {
			List<Employees> employeesList = employeesDAO.getEmployeesByPage(pageId, total);
			for (Employees employees : employeesList) {
				EmployeesModel employeesModel = new EmployeesModel();
				employeesModel.setEmployeeId(employees.getEmployeeId());
				employeesModel.setFullName(employees.getFirstName() + " " + employees.getLastName());
	...
	@Override
	public String updateEmployee(UpdatableEmployeesModel model) {
		Employees employees = new Employees();
		employees.setEmployeeId(model.getEmployeeId());
		employees.setEmail(model.getEmail());
		employees.setPhoneNumber(model.getPhoneNumber());
	...
	public int countNoOfEmployees() {
		int noOfEmployees = 0;
		try {
			noOfEmployees = employeesDAO.countNoOfRows();
		} catch (ClassNotFoundException | SQLException e) {
			log.error(e);
		}
